    ...
    client.onAddedToGroup(({ groupMetadata: { id }, contact: { name } }) =>
        client.getGroupMembersId(id)
            .then((ids) => {
                ...
                if (ids.length <= 10) {
                    client.sendText(id, 'Maaf Bro, anggota grup minimum adalah 10 pengguna untuk menggunakan bot ini. Selamat tinggal~').then(() => client.leaveGroup(id))
                } else {
                    client.sendText(id, `Hello group members *${name}*, terima kasih telah mengundang bot ini, untuk melihat menu bot kirim *#menu*`)
                }
            }))
